 ...
 *
 * @returns false, if switching is not possible (for example, because its a POST request)
 */
...
function csrfValidationFailed() {
    var errorMessage = Resource.msg('error.csrf.token.mismatch', 'common', null);
    if (request.httpParameterMap.format.stringValue === 'ajax') {
        let r = require('~/cartridge/scripts/util/responseUtil');
        r.renderJSON({
            error: errorMessage
        });
    } else {
